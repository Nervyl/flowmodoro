chrome.storage.local.get(["colors", "status", "time", "divisor"], (result) => {
  chrome.storage.local.set(
    ...
    () => {
      ...
      } else if (result.status === "work")
        startWork();
      else if (result.status === "break")
        startPause();
    }
  );
});
chrome.runtime.onMessage.addListener(async (request) => {
  ...
  if (request.action === "handleTimerClick")
    handleTimerClick();
});
  ...
  chrome.storage.local.get(["time"], (result) => {
    try {
      ...
      const updateTime = () => {
        ...
        chrome.storage.local.get(["status"], (result2) => {
          try {
            const currentTime = Date.now();
            const time = currentTime - startTime;
            chrome.storage.local.set({ time });
            if (result2.status !== "work")
              return;
  ...
  chrome.storage.local.get(["time", "divisor"], (result) => {
    try {
      ...
      const updateTime = () => {
        chrome.storage.local.get(["status"], (result2) => {
          try {
            if (result2.status !== "break")
              return;
            const currentTime = Date.now();
            const elapsedTime = currentTime - startTime;
            const time2 = breakTime - elapsedTime;
            chrome.storage.local.set({ time: time2 });
            chrome.runtime.sendMessage({ action: "updateTime", time: time2 });
            console.log("Sending message - Break", time2, status);
            if (time2 <= 0) {
              playAudio();
              startPause();
            }
